...
Main is served as two tiers — `main-high`/`main-low` — so `picam-frontend` can move a struggling browser viewer to a lighter one (see [Architecture](#architecture)). In the common case both are proxied straight through from picam-recorder's own always-live compression, which downscales `main-low` (not just a lower quality) to keep its compression cost sustainable — see picam-recorder's own README. This process only self-encodes main itself — always at native capture resolution, no downscale — for annotated mode or when OSD is burned into the live view, since those need pixels drawn only this process can draw. Lores is unrelated to any of this — a third, always-available, always-native-lores-resolution stream, always self-encoded here, used unconditionally for grid-view overview thumbnails regardless of connection quality. This process itself does no client-facing adaptation: every stream it serves is flat and pinned to whatever a client explicitly requested for the life of that connection — real connection-quality adaptation lives one hop further out, in `picam-frontend`, which has the actual variable-quality link (browser↔frontend); this process's own link to `picam-frontend` is LAN-only and effectively always clean.
